...
def has_cyrillic(text):
    """
    Checks string for containing Cyrillic symbols.
    """
...
def clear_text(text):
    """
    Clears string from trailong spaces, double spaces, etc.
    """
...
def get_journal_name_ru(odt_styles, journal_info):
    """
    Прототип функции получения названия журнала из стиля нижнего колонтитула.
    """
    footer_styles = odt_styles.find_all('style:footer')

    journal_names_ru_string = '|'.join(journal_info.journal_names_ru_list)
    regex_string = '(?:' + journal_names_ru_string + ')'

    # Собираем названия журнала в нижнем колонтитуле каждого стилей страниц
    # макета в cur_journal_names_list
    ...
    # Чтобы выбрать настоящее название журнала, вводим алгоритм выбора наиболее
    # часто встречающегося названия журнала.
    ...
    # Выбираем наиболее часто встречающееся название журнала
...
def get_info_from_citation_paragraph(odt_content, journal_info):
    ...
    for element in odt_content.find_all('text:p'):
        if 'Для цитирования' in element.get_text():
            ...
            if (result is None):
                print('Что-то не так со строкой для цитирования - {0}'.format(
                    element.get_text()))
                sys.exit(1)
...
def gather_journal_info(file_name):
    ...
    get_journal_name_ru(odt_styles, journal_info)
